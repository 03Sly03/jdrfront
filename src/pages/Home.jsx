    ...
    return (
        <div className={"flex bg-gray-700 w-[100vw] h-[100vh] p-[1vw] gap-[1vw]"}>
            <div className={"bg-yellow-500 w-[26vw] flex flex-wrap gap-2 p-[1vw] border-2 border-black"}>
                {data && data.map(charater => (
                    <div key={charater.id} className={"perso flex flex-col justify-center items-center gap-3"}>
                        <h2 className={"text-sm md:text-xl font-bold text-center"}>{charater.name.toUpperCase()}</h2>
                        ...
                        <Link state={{ id: charater.id }} to={`/perso/${charater.id}`} type="button" className={"border border-black w-[9vw] h-[4vh] bg-yellow-500 text-center"}>Infos</Link>
                    </div>
                ))}
